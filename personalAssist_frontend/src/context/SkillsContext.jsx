...
export const SkillsProvider = ({ children }) => {
    ...
    // Fetch all skills
    const fetchSkills = async (search = '', status = '', level = '') => {
        setLoading(true);
        setError(null);
        try {
    ...
    // Fetch full skill details (Topics, Subtopics, and Tasks)
    ...
    // Update skill
    const modifySkill = async (id, skillData) => {
        try {
    ...
    const addTopic = async (topicData) => {
        try {
            ...
            setTopics((prev) => [...prev, newTopic].sort((a, b) => a.orderIndex - b.orderIndex));
    ...
    const addSubtopic = async (subtopicData) => {
        try {
            ...
            setSubtopicsMap((prev) => ({
                ...prev,
                [topicId]: [...(prev[topicId] || []), newSubtopic].sort((a, b) => a.orderIndex - b.orderIndex),
            }));
    ...
    // Optimistic subtopic completion toggle
    const toggleSubtopic = async (subtopicId, topicId, completed) => {
        ...
        if (originalSkill) {
            ...
            Object.entries({ ...subtopicsMap, [topicId]: updatedSubtopics }).forEach(([_, list]) => {
                list.forEach((s) => {
                    total++;
            ...
            const optimisticProgress = total > 0 ? Math.round((completedCount / total) * 100 * 100) / 100 : 0.0;
            setCurrentSkill({
                ...originalSkill,
        ...
        try {
            ...
            const backendSubtopic = await api.toggleSubtopicComplete(subtopicId, completed);
            ...
            setSubtopicsMap((prev) => ({
                ...prev,
                [topicId]: prev[topicId].map((sub) => (sub.id === subtopicId ? backendSubtopic : sub)),
            }));
        } catch (err) {
            // Rollback on error
    ...
    const removeSubtopic = async (subtopicId, topicId) => {
        try {
    ...
    const toggleTask = async (taskId, subtopicId, completed) => {
        ...
        setTasksMap((prev) => ({
            ...
            [subtopicId]: prev[subtopicId].map((t) =>
                t.id === taskId
                    ? { ...t, status: completed ? 'COMPLETED' : 'TODO', completedAt: completed ? new Date().toISOString() : null }
                    : t
            ),
        }));
        ...
        try {
            ...
            setTasksMap((prev) => ({
                ...prev,
                [subtopicId]: prev[subtopicId].map((t) => (t.id === taskId ? updatedTask : t)),
            }));
        } catch (err) {
            // Rollback
    ...
    const removeTask = async (taskId, subtopicId) => {
        try {
